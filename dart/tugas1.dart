...
class TSP {
  ...
  Set<String> visitedNodes; // Menyimpan node yang sudah dikunjungi

  TSP(this.vertices) : visitedNodes = {}; // Inisialisasi visitedNodes
  ...
  List<String> findShortestPath() {
    ...
    for (var path in allPermutations) {
      // Reset visitedNodes for each path permutation
      visitedNodes.clear(); 
  ...
  void _markVisitsAndConnect(List<String> path) {
    for (var current in path) { // 'current' is the current city being processed
      if (!visitedNodes.contains(current)) { // Cek apakah kota sudah dikunjungi
        visitedNodes.add(current); // Tandai node yang sudah dikunjungi
