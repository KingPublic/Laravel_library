  ...
  for (int y = 0; y < height; y++) {
    ...
    for (int x = 0; x < width - 2; x++) {
      ...
      for (int i = 0; i < lizard.length; i++) {
        ...
        // Check for legs
        if (i > 0 && i < lizard.length - 1 && y == lizard[i][1] && (x == lizard[i][0] - 1 || x == lizard[i][0] + 1) && i % 2 == 0) {
          stdout.write("L");
          isLeg = true;
          break;
        }
...
// Add new part to the lizard body (between legs)
void addBodyPart() {
  int legIndex = lizard.length ~/ 4;
  List<int> newPart = lizard[legIndex];
  lizard.insert(legIndex, newPart);
}
...
void main() {
  while (true) {
    ...
    if (isFoodEaten()) {
      spawnFood();
      // Add a new part to the lizard's body (between legs)
      addBodyPart();
    }
